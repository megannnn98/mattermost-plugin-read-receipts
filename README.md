...
- Inline-индикатор сразу после текста сообщения — высоту поста не увеличивает:
  серая `✓` — доставлено, зелёные `✓✓` — прочитано, `✓✓ N` в неличных каналах
...
- **По умолчанию включены только личные сообщения (`D`)**. Группы, приватные и открытые каналы
  администратор включает явно через `EnabledChannelTypes` — обновление плагина не начинает
  собирать receipts по всей инсталляции само.
- **Треды вне scope этой версии**: у ответов в треде индикатор не рисуется и чтение не
  отправляется (ответ живёт в канале своего корня, поэтому его прочтение двигало бы канальный
  watermark по всем более старым сообщениям).
...
5. Настройте параметры:
   ...
   - **Enabled channel types**: типы каналов, по умолчанию `D`. `G`, `P`, `O` включаются явно.
...
- Нужен потому, что `pluginapi.WithPrefix` — это **клиентский фильтр поверх полного `KVList`**,
  то есть перебор всех ключей инсталляции; для вопроса «кто читал этот канал» он непригоден
- Пишется через CAS (`Atomic: true, OldValue: <прочитанные байты>`), **один раз на пару (канал,
  читатель)** — дальше на каждое чтение приходится ровно один `KVGet`
- Ретраев у этого CAS **64**, а не 5 как у watermark'а: проигравший CAS на watermark обычно
  выходит сразу («уже покрыто»), а каждый первый читатель канала обязан дописаться, поэтому
  число нужных попыток растёт с числом одновременных первых читателей. Исчерпание ретраев
  возвращает ошибку (`/read` → 500, клиент повторит по backoff), а не молча теряет читателя
- Индекс поддерживается **до** early-return по watermark-authority, поэтому установка,
  обновившаяся с `0.1.0` (есть `wm_`, нет `idx_`), самолечится на первом же чтении
- **Ушедшие из канала не учитываются**: читатели фильтруются по текущему членству
  (`GetChannelMembersByIds`, один вызов на страницу), а при заполнении индекса
  (`maxIndexReaders = 1000`) он чистится от вышедших — но только когда действительно заполнен,
  чтобы этот вызов не оплачивало каждое чтение
- **Страницы, а не молчаливая обрезка**: за пределами первых `maxQueryReaders = 200` ответ
  сообщает `truncated` и смещение следующей страницы, вместо того чтобы выдать префикс за весь канал

### Модель доступа

| | DM (`D`) | Группы, приватные, открытые (`G`/`P`/`O`) |
|---|---|---|
| Кто может спросить статус поста | автор поста | автор поста |
| Что возвращает `/receipts/query` | count + точное время | count + флаг усечения |
| Идентичности читателей в ответе | нет | нет |
| Список «кто прочитал» | `/receipts/post`, только автору | `/receipts/post`, только автору |
| Чужой пост в том же канале | нет статуса | нет статуса |
| Пост из другого канала | нет статуса | нет статуса |

Инвариант: **участник канала не может через API плагина узнать, кто и когда прочитал чужое
сообщение.** Он держится на трёх вещах, каждая из которых проверяется тестом:

1. `/receipts/query` отвечает только про посты, автором которых является сам запрашивающий, и
   авторство проверяется на сервере, а не берётся из запроса;
2. в ответе `/receipts/query` вообще нет ни идентичностей читателей, ни их read-позиций — только
   агрегат по своим постам, поэтому восстановить чужое состояние прочтения из него нечего;
3. `/receipts/post` — единственный источник поимённого списка — требует авторства поста.

Точное время в `/receipts/query` отдаётся только когда у канала ровно один читатель. Это
**правило стоимости**, а не безопасности: авторство уже проверено выше, а иначе ответ стоил бы
одного чтения на пост на читателя.
...
- счётчик агрегирует **сервер**: `/receipts/query` читает по одному watermark'у на читателя из индекса канала и считает, сколько водмарков покрыли `create_at` каждого своего поста (watermark монотонен, и получается без `GetPost` на один `post_id`). Клиенту в ответ не уходит ни одна личность читателя — только агрегат
...
- Пост существует
- Тип канала входит в `EnabledChannelTypes` (по умолчанию `D` — на upgrade сохраняется поведение 0.1.0)
...
```json
{
  "posts": {"<post_id>": {"count": 2, "truncated": false, "read_at": 0}},
  "truncated": false
}
```

Отвечает **только про собственные посты запрашивающего**. Авторство проверяется на сервере:
batch-API «дай эти post_id» в plugin API нет, поэтому вместо 200 `GetPost` канал прочитывается
страницами (`GetPostsForChannel`, не более `maxPostPages`), и запрошенные id сопоставляются с
тем, что вернулось. Пост чужой, из другого канала, удалённый или старше этих страниц — статуса
не получает.

`read_at` присутствует только когда у канала ровно один читатель (на практике DM) — это в
точности контракт `0.1.0`. `truncated` означает, что читателей больше, чем покрывает один
запрос, и любой `count` в ответе — нижняя граница.

`post_ids` ограничены (max 200, первые 200), невалидные id отбрасываются до любых обращений.
Тело запроса и ответа ограничены (64 KiB), весь трафик идёт с `X-CSRF-Token`/`X-Requested-With`.

**POST `/plugins/.../api/v1/receipts/post`**
```json
{"post_id": "...", "offset": 0}
```
→ `{"readers": [{"user_id": "...", "read_at": 0, "exact": true}], "truncated": false, "next_offset": 200}`

Вызывается при открытии списка «кто прочитал». Требует, чтобы запрашивающий **был автором**
...
Watermark читается **только когда точного receipt нет**. Ушедшие из канала не возвращаются.
`offset`/`next_offset` дают постраничный доступ ко всем читателям, а не только к первым 200.

**GET `/plugins/.../api/v1/config`**

→ `{"enabled_channel_types": "D"}`

Единственный источник истины для клиента: до его получения вебапп не рисует индикатор и не
отправляет чтения. Перечитывается на reconnect, поэтому изменение настройки администратором
применяется без перезагрузки клиента.
...
- Свой пост ещё отправляется (`pending_post_id == id`) или отклонён (`state == 'FAILED'`) → `null`
- Свой пост доставлен, никто не прочитал → серая `✓`
- Свой пост прочитан, DM → зелёные `✓✓`, точное время в `title`
- Свой пост прочитан, группа → зелёные `✓✓ N`, по клику — список «кто и когда»

**«Доставлено» не хранится.** Одинарная галочка означает «сервер принял сообщение» — тот же
факт, который на сервере даёт хук `MessageHasBeenPosted`. Клиент видит его сам: пост есть в
сторе и он не `pending`/`FAILED`. Поэтому состояние выводится, а не персистится: ни KV-ключа на
пост, ни дополнительного websocket-трафика.

**Галочки нарисованы, а не набраны.** Текстовый глиф наследует шрифт сообщения, поэтому его
размер и вес плывут вместе с темой, а его метрики затягиваются в строку абзаца. У SVG есть явный
бокс и цвет из тем-переменных Mattermost (`--online-indicator` для прочитанного,
`--center-channel-color` с прозрачностью для доставленного), так что индикатор читается и в
тёмной теме.
...
`inline_mount.ts` создаёт `<span>` и рендерит индикатор туда через `createPortal`. Если
контейнера текста нет (нестандартный вариант поста), используется запасная стратегия —
абсолютный оверлей в правом нижнем углу `.post__body` (он `position: relative`); высоту она тоже
не добавляет.

**Три вещи, которые нашлись только замером в живом клиенте** (ни одна не видна в jsdom):

1. Узел добавлялся **последним ребёнком `.post-message__text`**, то есть после блочного `<p>`
   сообщения. Inline-бокс после блока начинает новую анонимную строку — пост рос ровно на
   строку, которую эта правка и должна была убрать. Индикатор монтируется внутрь последнего
   абзаца; замыкающий код-блок или цитату не трогаем — галочка внутри них была бы хуже строки под ними.
2. После этого пост рос ещё на 2px: меньший шрифт с собственным `line-height` строит inline-бокс,
   half-leading которого свисает ниже strut'а абзаца. Обёртке задан `line-height: 0` — бокс
   схлопывается, глиф рисуется, высота строки остаётся ровно высотой абзаца.
3. Правка поста или реакция заставляли индикатор исчезнуть до перезагрузки: React перестраивает
   сообщение вместе с нашим узлом, а компонент — сосед сообщения, и его никто не будил.
   Render key поста входит в выбираемое состояние (это и будит компонент), а проверка
   `isConnected` пересоздаёт монтирование. Проверка смотрит в ref, а не в отрендеренное
   состояние: в том же коммите, где монтирование уже восстановлено, состояние ещё указывает на
   старый отсоединённый узел, и счётчик рос бы бесконечно.
...
Плагин не ограничивает число участников канала — это осознанное решение заказчика; какие типы
каналов собирают прочтения, задаёт администратор через `EnabledChannelTypes` (по умолчанию `D`
— только личные сообщения, чтобы на upgrade не начать молча собирать прочтения по всей
инсталляции; `GPO` администратор включает явно). Стоимость при этом зависит не от числа
участников канала, а от числа **реально читавших**, и ограничена сверху явными константами:

- **Людный открытый канал.** Каждый читающий попадает в `idx_`. Открытие канала автором стоит
  `min(K, 200)` чтений KV плюс один запрос членства, открытие списка читателей — до
  `2·min(K, 200)`. Сверх 200 ответ помечается `truncated`, счётчик показывается как `N+`, а
  список листается постранично. Аварийный тормоз для администратора — `EnabledChannelTypes`.
- **Индекс не имеет TTL.** Он чистится от вышедших из канала, но только когда заполнен
  (`maxIndexReaders`), и переживает выход-возврат участника. Канал, где 1000 разных людей
  читали и все остались, перестанет учитывать новых читателей: это осознанная граница.
- **Точное время в группах — только по клику.** До открытия списка видно количество, но не
  времена: массовый запрос иначе стоил бы одного чтения на пост на читателя.
- **Статус только для недавних постов.** Авторство проверяется через страницы канала
  (`maxPostPages`), поэтому свой пост, оказавшийся глубже этих страниц, статуса не получает —
  индикатор для него просто не рисуется.
