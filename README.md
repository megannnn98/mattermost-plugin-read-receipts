...
вьюпорт и никогда не дадут ratio 0.75. Для наблюдения этого fallback используется
плотный threshold-массив: `0`, ступени `0.001…0.0075`, затем шаг `0.01` до `1`.
Нижние ступени позволяют получить callback для очень высокого поста, когда доля
видимой области мала, но видимая полоса уже закрывает 75% viewport; поведение для
ещё более высоких постов консервативно — receipt не отправляется без callback.
